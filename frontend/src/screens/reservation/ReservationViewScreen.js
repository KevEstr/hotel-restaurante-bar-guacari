                ...
                {clientOrdersList && clientOrdersList.length > 0 ? (
                    clientOrdersList.map((order) => (
                        <div key={order.id} className="card mb-3">
                            ...
                            <div className="card-body">
                            <p>Fecha de la orden: <FormattedDate dateString={order.createdAt} /></p>
                            <p>Hora de la orden: <FormattedTime dateString={order.createdAt} /></p>
                            ...
                            <ul>
                                {order.products.map((product) => (
                                    <li key={product.id}>
                                        {product.name}
                                    <p>Cantidad: {product.OrderProduct.quantity}</p>
                                    <p>Precio: {product.price}</p>
                                    <p>Total: {order.total}</p>
                                    <p>Nota: {product.OrderProduct.note}</p>
                                    </li>
                                ))}
                            </ul>
                ...
                <div className="card">
                    ...
                    <div className="card-body">
                        <div className="row">
                            ...
                            <div className="col-md-6">
                            {reservation && reservation.room && reservation.room.length > 0 && (
                                <div>
                                    {reservation.room.map((room, index) => (
                                        <div key={index}>
                                            <h5>ID de habitación:</h5>
                                            <p>{room.id}</p>
                                            <h5>Nombre de la habitación:</h5>
                                            <p>{room.name}</p>
                                            {/* Puedes agregar más detalles de la habitación aquí si es necesario */}
                                        </div>
                                    ))}
                                </div>
                            )}
