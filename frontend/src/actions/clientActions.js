...
export const listClients = (keyword = "", pageNumber = "", hasReservation = false, isActive = true, includeInactive = false) => async (dispatch, getState) => {
    try {
        dispatch({
        ...
        let url = `/api/clients?keyword=${keyword}&pageNumber=${pageNumber}&is_active=${isActive}`;
        if (hasReservation) {
