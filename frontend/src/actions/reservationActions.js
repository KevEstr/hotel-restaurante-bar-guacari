...
export const createReservation = (reservation) => async (dispatch, getState) => {
    const { price, start_date, end_date, note, quantity, clientId, rooms, paymentId, is_paid, total} = reservation;
    ...
    try {
        ...
        //create category
        const { data } = await axios.post("/api/reservations", { price, start_date, end_date, note, quantity, clientId, rooms, paymentId, is_paid, total }, config);
        dispatch({
